...
const searchField = ["title", "description", "amount", "expense_date"];
...
const headers = [
  ...
  { text: "expense_date", value: "expense_date", sortable: true },
  { text: "operations", value: "operations" },
];
const addExepenseVisible = ref(false);
const toast = useToast();
const showlExpenseVisible = ref(false);
const editExpenseVisible = ref(false);
let itemToShow;
const showlExpense = (item) => {
  itemToShow = item;
  showlExpenseVisible.value = true;
};
let itemToEdit;
const editExpense = (item) => {
  itemToEdit = item.id;
  expenseForm.title = item.title;
  expenseForm.description = item.description;
  expenseForm.amount = item.amount;
  expenseForm.expense_date = item.expense_date
...
const addExepense = () => {
  if (
    ...
    router.post("/expenses", expenseForm, {
      ...
      onProgress: (page) => { },
      onSuccess: (page) => {
        addExepenseVisible.value = false;
        expenseForm.reset;
...
const editExpenseFun = () => {
  if (
    ...
    router.put("/expenses/" + itemToEdit, expenseForm, {
      preserveScroll: true,
      preserveState: true,
      onProgress: (page) => { },
      onSuccess: (page) => {
  ...
  <InsideLayout>
    <div class="bg-white p-6 m-4 rounded-xl grid xl:grid-cols-4">
      ...
      <div class="flex justify-start lg:m-2 xl:m-0">
        <button label="Show" @click="openAddExpenseModal"
          ...
          </svg><span class="mx-2">{{ $t('buttons.addexpenses') }}</span>
        </button>
      </div>
      <div class="flex mt-2">
    ...
    <div class="m-4" :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
      <Vue3EasyDataTable :headers="headers" :items="expenses" :search-field="searchField" :search-value="searchValue"
        ...
        <template #header-description="header">
          <p class="text-white text-[1.2rem] py-2">
            {{ $t('tableexpenses.description') }}
          </p>
        </template>

        <template #header-amount="header">
          <p class="text-white text-[1.2rem] py-2">
            {{ $t('tableexpenses.amount') }}
          </p>
        </template>

        <template #header-expense_date="header">
          <p class="text-white text-[1.2rem] py-2">
            {{ $t('tableexpenses.expense_date') }}
          </p>
        </template>
        ...
        <template #item-operations="item" :class="$i18n.locale === 'ar' ? 'justify-start' : 'justify-start'">
          <div class="flex justify-center">
            <button @click="showlExpense(item)">
            ...
            <button @click="editExpense(item)" class="mx-2">
    ...
    <div class="card flex justify-content-center m-2">
      <Dialog v-model:visible="addExepenseVisible" modal :header="$t('buttons.addexpenses')" :style="{ width: '50rem' }"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <div :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
          <div class="m-10">
            <form class="grid" @submit.prevent="addExepense">
              <label for="title" class="mt-2 text-gray-500">{{ $t('tableexpenses.title') }}</label>
              <input type="text" class="w-full h-14 rounded-xl border border-gray-200 my-2"
                :placeholder="$t('tableexpenses.title')" id="title" v-model="expenseForm.title" />




              <label for="amount" class="mt-2 text-gray-500">{{ $t('tableexpenses.amount') }}*</label>
              <input type="number" class="w-full h-14 rounded-xl border border-gray-200 my-2"
                :placeholder="$t('tableexpenses.amount')" id="amount" v-model="expenseForm.amount" />

              <label for="income_date" class="mt-2 text-gray-500">{{ $t('tableexpenses.expenses_date') }} *</label>
              <input type="date" class="w-full h-14 rounded-xl border border-gray-200 my-2"
                :placeholder="$t('incometable.income_date')" id="expense_date" v-model="expenseForm.expense_date" />
              <label for="description" class="mt-2 text-gray-500">{{ $t('tableexpenses.description')
              }}description*</label>
    ...
    <!--EDit LOCALE MODAL-->
    <div class="card flex justify-content-center m-2">
      <Dialog v-model:visible="editExpenseVisible" modal :header="$t('buttons.editexpenses')" :style="{ width: '50rem' }"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <div :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
          <div class="m-10">
            <form  class="grid" @submit.prevent="editExpenseFun">
              
              <label for="title" class="mt-2 text-gray-500">{{ $t('tableexpenses.title') }}*</label>
              <input type="text" class="w-full h-14 rounded-xl border border-gray-200 my-2"
                :placeholder="$t('tableincome.title')" id="title" v-model="expenseForm.title" />

              <label for="amount" class="mt-2 text-gray-500">{{ $t('tableexpenses.amount') }}*</label>
              <input type="text" class="w-full h-14 rounded-xl border border-gray-200 my-2"
                :placeholder="$t('tableincome.amount')" id="amount" v-model="expenseForm.amount" />
              <label for="income_date" class="mt-2 text-gray-500">{{ $t('tableexpenses.expenses_date') }}*</label>
              <input type="date" class="w-full h-14 rounded-xl border border-gray-200 my-2"
                :placeholder="$t('tableincome.expenses_date')" id="income_date" v-model="expenseForm.expense_date" />
              <label for="description" class="mt-2 text-gray-500">{{ $t('tableexpenses.description') }} *</label>
              <textarea type="text" class="w-full rounded-xl border border-gray-200 my-2"
                :placeholder="$t('tableincome.description')" id="description" rows="6"
                v-model="expenseForm.description"></textarea>
              ...
              <div class="flex justify-end mt-5">
                <button
                  ...
                  </svg><span class="mx-2">{{ $t("buttons.editexpenses") }}</span>
                </button>
              </div>
            </form>
          </div>
    ...
    <div class="card flex justify-content-center m-2">
      ...
      <Dialog v-model:visible="showlExpenseVisible" header="Income" modal :style="{ width: '50rem' }"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
        <label for="title" class="text-x font-semibold text-[#6F32BE]">{{ $t('tableexpenses.title') }}*</label>
        <input class="w-full h-10 rounded-xl border border-gray-200 my-2" :placeholder="itemToShow.title"
          :disabled="inputDisabled" />


        <label for="amount" class="text-x font-semibold text-[#6F32BE]">{{ $t('tableexpenses.amount') }}*</label>
        <input type="number" class="w-full h-10 rounded-xl border border-gray-200 my-2" :placeholder="itemToShow.amount"
          :disabled="inputDisabled" />

        <label for="expense_date" class="text-x font-semibold text-[#6F32BE]">{{ $t('tableexpenses.expenses_date')
        }}*</label>
        <input class="w-full h-10 rounded-xl border border-gray-200 my-2" :placeholder="itemToShow.expense_date"
          :disabled="inputDisabled" />


        <label for="description" class="text-x font-semibold text-[#6F32BE]">{{ $t('tableexpenses.description')
        }}*</label>
        <textarea type="text" class="w-full rounded-xl border border-gray-200 my-2" :placeholder="itemToShow.description"
          id="description" rows="6" :disabled="inputDisabled" v-model="expenseForm.description"></textarea>
